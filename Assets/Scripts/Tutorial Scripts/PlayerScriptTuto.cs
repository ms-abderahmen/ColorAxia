...
public class PlayerScriptTuto : MonoBehaviour {
	...
	[HideInInspector]
	public bool[] doneMoving = new bool[5];
	[HideInInspector]
	...
	void Start () {
		...
		for (int i = 0; i < doneMoving.Length; i++) {
			doneMoving [i] = false;
		}
	}
	...
	void Update () {
		if (GameManagerScript.stopMoving == false) {
			...
			for (int i = 0; i < doneMoving.Length-1; i++) {
				if (doneMoving [i] == false) {
					doneMoving [4] = false;
					break;
				}
				if ((i == 3) && (doneMoving [i] == true)) {
					doneMoving [4] = true;
				}
			}
		}
	...
	void ScoreCalculation(Transform pos){
		...
		showText (pos, count.ToString(), Color.red);
		stage.scores[playerNumber] += count;
	}

	public void showText(Transform pos,string text,Color color){
		//Score text script
